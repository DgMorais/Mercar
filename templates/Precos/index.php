<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\Preco[]|\Cake\Collection\CollectionInterface $precos
 */
?>
<div class="precos index content">
    <?= $this->Html->link(__('New Preco'), ['action' => 'add'], ['class' => 'button float-right']) ?>
    <h3><?= __('Precos') ?></h3>
    <div class="table-responsive">
        <table>
            <thead>
                <tr>
                    <th><?= $this->Paginator->sort('id') ?></th>
                    <th><?= $this->Paginator->sort('product_id') ?></th>
                    <th><?= $this->Paginator->sort('preco_de') ?></th>
                    <th><?= $this->Paginator->sort('preco_por') ?></th>
                    <th><?= $this->Paginator->sort('status') ?></th>
                    <th><?= $this->Paginator->sort('created') ?></th>
                    <th><?= $this->Paginator->sort('modified') ?></th>
                    <th class="actions"><?= __('Actions') ?></th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($precos as $preco): ?>
                <tr>
                    <td><?= $this->Number->format($preco->id) ?></td>
                    <td><?= $this->Number->format($preco->product_id) ?></td>
                    <td><?= $preco->preco_de === null ? '' : $this->Number->format($preco->preco_de) ?></td>
                    <td><?= $this->Number->format($preco->preco_por) ?></td>
                    <td><?= h($preco->status) ?></td>
                    <td><?= h($preco->created) ?></td>
                    <td><?= h($preco->modified) ?></td>
                    <td class="actions">
                        <?= $this->Html->link(__('View'), ['action' => 'view', $preco->id]) ?>
                        <?= $this->Html->link(__('Edit'), ['action' => 'edit', $preco->id]) ?>
                        <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $preco->id], ['confirm' => __('Are you sure you want to delete # {0}?', $preco->id)]) ?>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
    </div>
    <div class="paginator">
        <ul class="pagination">
            <?= $this->Paginator->first('<< ' . __('first')) ?>
            <?= $this->Paginator->prev('< ' . __('previous')) ?>
            <?= $this->Paginator->numbers() ?>
            <?= $this->Paginator->next(__('next') . ' >') ?>
            <?= $this->Paginator->last(__('last') . ' >>') ?>
        </ul>
        <p><?= $this->Paginator->counter(__('Page {{page}} of {{pages}}, showing {{current}} record(s) out of {{count}} total')) ?></p>
    </div>
</div>
